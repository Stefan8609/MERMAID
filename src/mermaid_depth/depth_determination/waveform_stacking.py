...
def grid_search_ocean_depth(
    ...
    """
    Grid search over ocean depth H using stacked seafloor reverberation packets.

    Returns the same dictionary structure as grid_search_ocean_depth2D. The 1D
    search is represented as a degenerate 2D search with one fixed t0 value, so
    scores has shape:
    ...
    first_arrival_time_grid = np.asarray([first_arrival_time], dtype=float)
    ...
    if best is None:
        raise ValueError("No valid depth found. Try fewer reverbs, larger t_max, or shorter windows.")
...
def plot_depth_search(
    ...
    """Plot waveform, best stacked packets, and 1D depth-search score."""

    best = search_result["best"]
    H_grid = search_result["H_grid"]
    first_arrival_time_grid = search_result["first_arrival_time_grid"]
    scores = np.asarray(search_result["scores"], dtype=float)

    expected_shape = (len(first_arrival_time_grid), len(H_grid))
    if scores.shape != expected_shape:
        raise ValueError(
            f"scores has shape {scores.shape}, but expected {expected_shape}."
        )
    if len(first_arrival_time_grid) != 1:
        raise ValueError("plot_depth_search expects a 1D search with one fixed t0.")

    scores_1d = scores[0]
    score_max = np.nanmax(scores_1d)
    ...
    score_norm = scores_1d / score_max
    misfit = 1.0 - score_norm
    first_arrival_time = best["first_arrival_time"]
    ...
    # Direct arrival: this is also best["bottom_times"][0] in the coherence search.
    axes[0].text(
        first_arrival_time + 0.01 * x_span,
        y_label_direct,
        r"$w^{s,b}_{0,0}$",
        rotation=0,
        ha="left",
    ...
    # Seafloor/bottom reverberation packet starts. Skip n = 0 because it is
    # the direct arrival, even though it is included in the coherence search.
    ...
    # Surface reverberations. For n = 0, this is the first surface echo:
    # w^{s,b}_{1,0}.
    ...
    # H-grid score / misfit
    # ------------------------------------------------------------
    axes[2].plot(H_grid, score_norm, label="normalized stack score")
    axes[2].plot(H_grid, misfit, label="misfit = 1 - normalized score")

    axes[2].axvline(
        best["H"],
        linestyle="--",
        label=f"best H = {best['H']:.1f} m",
    )

    axes[2].axvline(
        H0,
        linestyle="--",
        label=f"GEBCO H = {H0:.1f} m",
    )

    axes[2].grid(True)
    axes[2].set_xlabel("trial ocean depth H (m)")
    axes[2].set_ylabel("score / misfit")
    axes[2].legend()
...
if __name__ == "__main__":
    ...
    print(f"Difference best - GEBCO: {best['H'] - H0:.1f} m")
